...
def test_nine_archival_facts_are_user_approved_and_exactly_bound_to_config() -> None:
    ...
    for fact in lock["facts"].values():
        ...
        assert evidence["kind"] == "user_approval"
        assert evidence["approved_by"] == "user"
        approval = PROJECT_ROOT / evidence["path"]
        assert evidence["sha256"] == hashlib.sha256(approval.read_bytes()).hexdigest()
    ...
    assert lock["facts"]["frame_sampling"]["selected_value"] == {
...
def test_preprocessing_lock_freezes_official_t10_keyframe_and_audio_semantics() -> None:
    config = _yaml(PROJECT_ROOT / "configs/ov_orthkd_mm26_repro.yaml")
    ...
    receipt_lock = lock["official_archive_observation"]["receipt"]
    receipt_path = PROJECT_ROOT / receipt_lock["path"]
    assert receipt_lock["sha256"] == hashlib.sha256(receipt_path.read_bytes()).hexdigest()
...
def test_hash_locked_text_evidence_is_checked_out_with_lf_bytes() -> None:
    evidence_paths = [
        "reports/data/official_preprocessed_download_receipt.json",
        "reports/data/preprocessed_layout_discovery.json",
        "reports/archival/R3_USER_APPROVED_RECONSTRUCTION.md",
        "reports/archival/R4_USER_APPROVED_TEMPORAL_PROTOCOL.md",
