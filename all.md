...
### 826. 2026-09-03: SSH reset at epoch 4 and Task Scheduler handoff

- The persistent SSH session ended with `client_loop: send disconnect: Connection reset` after epoch 3 had completed and epoch 4 reached approximately batch `90`. A retrying SSH query initially timed out, then `nvidia-smi` showed the GPU idle and no training process. The completed epoch-3 checkpoint remained intact; no epoch-4 history/checkpoint was written.
- A Windows Task Scheduler detach probe was registered under the logged-in account `desktop-lpn6mt3\\lxt`, observed `Running` from a separate SSH connection after the creating SSH command exited, and then removed. This confirms Task Scheduler processes are not tied to the SSH job object.
- The unchanged training command was registered and started as task `OVOrthKD_VisualSum_Seed42_NoWorkers` with the same no-worker config, seed, output directory, resume checkpoint, and `--allow-blocked-reproduction`; settings include a 3-day execution limit, ignore-new multiple instances, and up to three 2-minute restarts. A separate SSH check at `20:07:58` found the task `Running`, Python child PID `4824` alive, 5090 memory allocated, and `history_lines=3` with `last.pt` updated at `16:31:22`. Future SSH disconnects should no longer terminate this scheduled task; only machine/user-session failures remain external risks.

### 827. 2026-09-03: task-action failure diagnosed and corrected
...
### 830. 2026-09-03: elevated task passed startup and entered epoch 4

- At `20:18:00`, the elevated scheduled task remained `Running` with Python parent/child processes `15400`/`5036`; 5090 memory was about `7.3 GB` and no new exception appeared. `task_stderr.log` now contains the normal `epoch 4/30: 0%` progress line, showing that the process passed the previously failing official-JPG permission boundary and resumed training from epoch 3. `history_lines` remains `3` until epoch 4 reaches full validation.

### 831. 2026-09-04: 30-epoch Visual-only sum control completed and independently verified

- A fresh 5090 audit at `12:54:08` found scheduled task `OVOrthKD_VisualSum_Seed42_NoWorkers` in the normal post-completion `Ready` state with `LastTaskResult=0`, no remaining training process, and an idle GPU. `history.jsonl` contains exactly `30` epoch records through epoch index `29` / global step `12000`; `final_metrics.json`, `best.pt`, `last.pt`, and `train.log` are present. The final evaluation finished at `05:56:06`.
- The best validation checkpoint was epoch index `7` (human epoch 8), not the final epoch: validation AP `0.7534809404354474`, AUROC `0.6540250203878859`, predicted-positive rate `0.7779061745429459`, segment F1 `0.5577574807678815`, and event F1 `0.5589168678854777` at threshold `0.5`.
- Evaluating that best checkpoint on the full test split produced AP `0.7545760852128325` and AUROC `0.6565048482408069`. At threshold `0.5`, segment/event F1 are `0.5630208433178135` / `0.5613766159384715`; seen/unseen AP are `0.8054709125479085` / `0.729366810549263`. The validation-calibrated threshold is `0.4358334281623338`.
- The last epoch regressed to validation AP `0.658511371286369`, AUROC `0.5430195611237815`, and predicted-positive rate `1.0`. Even the best checkpoint predicts positive for `0.9824742268041237` of test segments at the calibrated threshold. Operational completion is therefore PASS, but the scientific shortcut/collapse is not repaired: the Eq. 9-style visual-feature `sum` control improves ranking over the prior mean-reduction Visual-only run, yet remains below the conference Visual-only AP `0.778` and does not authorize a formal Full run.

### 832. 2026-09-04: post-hoc attribution runner implemented and locally reviewed

- The external diagnosis was technically checked against the current code and accepted only as a staged, read-only investigation: the completed best `Visual-only sum` checkpoint must be tested for mixed-label visual/audio/both zeroing, audio temporal shuffle, per-query and seen/unseen AP, pairwise concordance, and projector/decision geometry before any bounded Full consideration. Formal Full remains blocked.
- A written implementation plan was added at `docs/superpowers/plans/2026-09-04-visual-sum-posthoc.md`. TDD was followed: the new five-test module first failed at collection because the runner interface did not exist, then the minimal runner implementation made the focused suite pass (`5 passed`). Combined focused regression checks returned `28 passed, 1 skipped`; `compileall` and Ruff both exited `0`.
- The runner is `scripts/diagnose_visual_sum_posthoc.py` with tests in `tests/test_visual_sum_posthoc.py`. It is locked to official T=10, uses no optimizer or checkpoint writes, evaluates interventions only on mixed-label test samples, compares fresh original logits with the saved archive, and always reports `formal_full_training_authorized=false`.
- A standard local full-suite attempt exited `1` during NumPy/MKL import; retrying with the documented duplicate-OpenMP compatibility override reached collection but found `timm` absent in the lightweight local environment (`23` collection errors). This is recorded as an environment limitation; full verification will use the locked 5090 environment that produced the source checkpoint.

### 833. 2026-09-04: persistent post-hoc task launched on 5090

- The first Task Scheduler registration attempt failed before task creation because the Windows PowerShell enum name `InteractiveToken` is invalid on this host (`New-ScheduledTaskPrincipal` accepts `Interactive`, among other values). No post-hoc task existed after that failure; the unrelated completed training task remained `Ready`.
- The registration was corrected using the existing logged-in account SID `S-1-5-21-1354097161-55421191-3342400216-1001`, `LogonType=Interactive`, `RunLevel=Highest`, `StartWhenAvailable`, `IgnoreNew` multiple-instance policy, 24-hour execution limit, and three two-minute restart attempts. Task `OVOrthKD_VisualSum_Posthoc_Seed42` started successfully in `Running` state.
- The action is read-only: it invokes `scripts/diagnose_visual_sum_posthoc.py` with the locked resolved config, best/last checkpoints, saved validation/test predictions, training diagnostics, official T=10 checks, 100 audio-shuffle repeats, and atomic JSON output under `outputs/diagnostic/visual_sum_posthoc_seed42`; no optimizer, training, checkpoint, or formal-Full authorization is enabled.

### 834. 2026-09-04: post-hoc attribution completed and independently audited

- The persistent 5090 task completed with Task Scheduler result `0`; Python exited normally, `task_stderr.log` is empty, and the remote report and stdout are each `57,588` bytes. The report was copied locally and the remote/local report SHA256 matched: `32c2d11e33da2e0c4390cf81dda33e03d398c1d67120d36f223c411c5d11da56`.
- The machine-readable report passed an independent audit: status `PASS`, official `task_segments=10`, 1,941 mixed-label samples / 19,410 segments, validation-calibrated threshold `0.4358334281623338`, fresh-vs-archive best logits max absolute difference `2.38418579101563e-07`, best/last steps `3200/12000`, duplicate step `400` explicitly ignored, and `optimizer_constructed=false`, `optimizer_step_executed=false`, `checkpoint_written=false`, `formal_full_training_authorized=false`, `next_experiment_authorized=false`.
- Mixed-label visual-zero changed AP by only `-0.000132269` and AUROC by `-0.000028813`; audio-zero changed AP/AUROC by `+0.004835916/+0.002390646`; audio temporal shuffle produced mean AP/AUROC drops `0.000661118/0.003176299` over 100 within-sample repeats. Pairwise concordance was original `0.553534304`, visual-zero `0.554686183`, audio-zero `0.537829409`, both-zero `0.534050683`, and audio-shuffle `0.517699612`.
- Best-checkpoint strong teacher-projector relative drift was `0.257007078` (last `0.315639318`); decision variance on the fixed mixed batch was `4.74677e-05` (last `8.55843e-06`). The scientific interpretation is that the Visual-only sum run is operationally valid but still does not show substantive visual decision dependence or restore the full Student-only < Visual-only < Full relation. Formal Full remains paused. The detailed report is `reports/formal_reproduction/student_shortcut_recovery/VISUAL_SUM_POSTHOC_ATTRIBUTION.md` with JSON and stdout evidence beside it.
- The report markdown itself was independently checked against the JSON, link targets exist, and `git diff --check` passed. The report states the required unresolved provenance items (target-projector freezing, shared query anchor, pretrained backbone, additive-vs-concat structure, step400/early-stop schedule, and U-AP aggregation) before any bounded Full consideration.

### 835. 2026-09-04: final code and evidence verification before commit
...
### 1016. 2026-09-10: D2 zero-training gate authorization and immutable-input preflight

- The user explicitly authorized the preregistered D2 zero-training scientific gate. The boundary remains train-fit/validation-eval only: seed 42, official `T=10`, the existing 256-record-per-split subset budget, no test split, no neural optimizer, no backward/update, no model checkpoint, no 800-step D2/D3/Full run, no second seed, and no schedule extension.
- An initial SSH inspection was quoted incorrectly and allowed local PowerShell commands to report the workstation RTX 4070; that invocation found no remote assets and made no mutation. An isolated encoded-command recheck proved the actual target is `DESKTOP-LPN6MT3`, `NVIDIA GeForce RTX 5090`, GPU UUID `GPU-eaa0b0b1-d8c9-ecd2-4f66-8afa473fef68`, 32,607 MiB, 488 MiB used, 0% utilization, with no relevant Python/pytest process.
- The clean tested remote snapshot is `E:/OV-OrthKD-R3/d2-e0-1-d7b29ca/repo` (isolated Git HEAD `dadc8e1aa49d2009f58620a8900a2e6224e15bd2`, empty status). Runtime is Python 3.11.9, torch 2.10.0+cu128, timm 1.0.28, safetensors 0.8.0.
- Locked scientific inputs were rehashed before launch: probe script `c57d69ea3c4c80ad53bb459d55c283bdc539000229f9cf1c1e37f98b599fde1e`; D2 wrapper `b9c32df24aef4c30e2ca5869b52c5dbc16a4c517ab46fe35332f2e548110643d`; C2 `best.pt` 562,516,965 bytes / `1b997952d2f9bb89b3783352655b044671a72a84ed76dcd6c4b8be6bf29cecf0`; train manifest `cb30035c533d56d44469d063ba11720ae3660266535ede670db6b6f53bdc7666`; validation manifest `df2e8979c3fa05dcadaeb5ff7ef9726263fae7950b7478d30cb709aefbc97160`; exact model 114,561,694 bytes / `6652fd90fc9c23977659e58515778e16fbbcd43f0b01fc693089cebe6d64c2a9`; exact config 901 bytes / `a9adf5b660f85c08af085b19a80a47b942f574e62211f7af6c3c7b6fe2a88eb6`.

### 1017. 2026-09-10: durable D2 zero-training gate execution

- A first attempt to transmit the complete worker inline was rejected by the Windows command-length limit before remote directory/process creation. Added the auditable `run_d2_zero_training_gate_20260910.ps1`; PowerShell parser and boundary inspection passed, and the local/remote runner SHA256 matched at `c9a5cbb5426f3f6390c396395eb5771f187114f49889337bcc5259ff40742527`.
- Created the fresh isolated control root `E:/OV-OrthKD-R3/d2_zero_training_gate_20260910` and launched a hidden persistent worker. One launcher SSH client retained an inherited handle after the 30-second observation window; only that locally created SSH client was stopped. The remote PowerShell/Python process tree remained alive and was monitored independently; no duplicate launch occurred.
- The exact command used the reviewed D2 wrapper, C2 best checkpoint, train/validation manifests, `--sample-count 256`, and `--seed 42` with Hugging Face/Transformers offline variables. It started at `2026-09-10T02:19:08Z`, completed at `02:24:48Z`, exited `0`, wrote a 33,683-byte JSON, and left zero-byte stderr. GPU use returned to 488 MiB / 0%; no relevant process remained.

### 1018. 2026-09-10: corrected D2 scientific result and remote artifact closure

- Corrected aligned/shared-map values are QP `0.49424307036247334`, random-visual VQP `0.4906183368869936`, exact-pretrained VQP `0.5127931769722814`, current-C2 VQP `0.5174840085287846`, and audio-positive-control VQP `0.6788912579957356` mixed concordance.
- Independent gate arithmetic gives pretrained-minus-random `+0.02217484008528786` versus required `+0.05` (margin `-0.027825159914712128`) and pretrained-minus-QP `+0.018550106609808104` versus required `+0.02` (margin `-0.0014498933901918587`). Both conjunctive requirements fail: `VISUAL_PRETRAINING_CONTROL_FAIL`; D2 800-step is not authorized.
- Remote/local result SHA256 matched at `5f29e4ca172334e9e041cac7e57ca11c6adb9d5d5686c7629cd8435d680f898c`; input/state/exit/stdout/stderr hashes matched at `3da5a39...7774f`, `fa670699...0839`, `13bf7b30...0354`, `29be4e2f...3a53`, and empty-file `e3b0c442...b855`. A first PowerShell `-Include` inventory misleadingly counted all seven control files as model artifacts; the corrected extension-filtered inventory found exactly zero `.pt/.pth/.ckpt/.npz` files, no subset directory, and no process.
- Collected all compact artifacts into both the repository evidence directory and `扩刊/复现/student_shortcut_recovery/d2_zero_training_gate_20260910`; the large checkpoint, official data, and pretrained bytes remain remote-only.

### 1019. 2026-09-10: independent D2 artifact auditor and status publication
