...
> Current diagnostic status (2026-09-10): engineering integrity passes, current-T10 direct visual logits carry healthy aligned boundary signal, and visual feature loss is mean-component dominated under strong clipping. D1 centered-visual failed. D2's exact offline asset/load/parity checks passed, but its corrected validation-only zero-training superiority gate also failed (`C_pretrained=0.512793`, only `+0.022175` over random and `+0.018550` over QP). The repaired D3 visual-logit control remains unexecuted because its positive analysis weight has no archival provenance. The precise state is `NO_EXECUTED_BOUNDED_CONTROL_RECOVERS_BOUNDARY`; D2 800-step and formal Full remain on hold. Start with the [D2 gate report](reports/formal_reproduction/student_shortcut_recovery/D2_ZERO_TRAINING_GATE_REPORT.md) and [closure handoff](reports/formal_reproduction/student_shortcut_recovery/TEACHER_SIGNAL_CLOSURE_FINAL.md).
...
> **Canonical run record:** the seed42 run completed on the RTX 5090 for 30 epochs / 12,000 optimizer steps and its full artifact audit passed with zero errors. The paper's numerical result was **not** reproduced: test AP/AUROC/OV-AVEL segment F1@0.5 are `0.741946 / 0.633875 / 0.540393`, versus `0.816 / 0.750 / 0.596` in the paper. The latest bounded-diagnostic state is recorded above: D1 failed, and D2's corrected zero-training scientific gate completed with artifact PASS but scientific FAIL. Repaired D3 is unexecuted pending a provenance-backed positive weight. No second seed, schedule extension, new test evaluation, D2 training, or formal Full has been started. Read [CURRENT_STATUS.md](CURRENT_STATUS.md) and the [formal-reproduction evidence package](reports/formal_reproduction/README.md) before launching more runs.
