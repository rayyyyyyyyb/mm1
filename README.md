...
> Current diagnostic status (2026-09-09): engineering integrity passes, current-T10 direct visual logits carry healthy aligned boundary signal, and visual feature loss is mean-component dominated under strong clipping. Only D1 centered-visual was executed and it failed; D2 has a locally verified exact asset but remains blocked from target-5090 installation/verification, while the repaired D3 visual-logit control remains unexecuted because its positive analysis weight has no archival provenance. The precise state is `NO_EXECUTED_BOUNDED_CONTROL_RECOVERS_BOUNDARY`; formal Full remains on hold. Start with the [closure handoff](reports/formal_reproduction/student_shortcut_recovery/TEACHER_SIGNAL_CLOSURE_FINAL.md) and [D3 correction](reports/formal_reproduction/student_shortcut_recovery/D3_VISUAL_LOGIT_ORCHESTRATION_CORRECTION.md).
...
> **Canonical run record:** the seed42 run completed on the RTX 5090 for 30 epochs / 12,000 optimizer steps and its full artifact audit passed with zero errors. The paper's numerical result was **not** reproduced: test AP/AUROC/OV-AVEL segment F1@0.5 are `0.741946 / 0.633875 / 0.540393`, versus `0.816 / 0.750 / 0.596` in the paper. The latest bounded-diagnostic state is recorded above: D1 is the only executed control and failed; D2's exact offline asset/load/parity checks pass and its corrected zero-training probe is ready but unrun; repaired D3 is unexecuted pending a provenance-backed positive weight. No second seed, schedule extension, test evaluation, D2 training, or formal Full has been started. Read [CURRENT_STATUS.md](CURRENT_STATUS.md) and the [formal-reproduction evidence package](reports/formal_reproduction/README.md) before launching more runs.
